...
public class Rasterization {
    ...
    public static void fillTriangle(
            DrawUtilsJ gr, MyPoint3D p1, MyPoint3D p2, MyPoint3D p3,
            MyColor myColor1, MyColor myColor2, MyColor myColor3,
            Double[][] zBuffer, Integer[][] triangleBuffer, Camera camera,
            BufferedImage image, Vector2f texturePoint1, Vector2f texturePoint2,
            Vector2f texturePoint3, Model mesh, int triangleIndex) throws IOException {
        ...
        for (int y = (int) (y1 + 1); y <= y2; y++) {
            ...
            fillLine(gr, y, startX, endX, myColor1, myColor2, myColor3, x1, x2, x3, y1, y2, y3, z1, z2, z3, zBuffer, triangleBuffer, camera, cosLight, image, texturePoint1, texturePoint2, texturePoint3, mesh, triangleIndex); }
    ...
    public static void fillTriangle(
            ...
            MyColor myColor1, MyColor myColor2, MyColor myColor3,
            Double[][] zBuffer, Integer[][] triangleBuffer, Camera camera, BufferedImage image,
            Vector2f texturePoint1, Vector2f texturePoint2, Vector2f texturePoint3, Model mesh, int triangleIndex) throws IOException {
        fillTriangle(gr, new MyPoint3D(x1, y1, z1), new MyPoint3D(x2, y2, z2), new MyPoint3D(x3, y3, z3),
                myColor1, myColor2, myColor3, zBuffer, triangleBuffer, camera, image, texturePoint1, texturePoint2, texturePoint3, mesh, triangleIndex);
    }
    ...
    private static void fillLine(
            DrawUtilsJ gr, int y, double startX, double endX,
            MyColor myColor1, MyColor myColor2, MyColor myColor3,
            double x1, double x2, double x3, double y1, double y2, double y3,
            double z1, double z2, double z3, Double[][] zBuffer,
            Integer[][] triangleBuffer, Camera camera, double cosLight,
            BufferedImage image, Vector2f texturePoint1, Vector2f texturePoint2,
            Vector2f texturePoint3, Model mesh, int triangleIndex) throws IOException {
        ...
        for (int x = (int) startX + 1; x < endX; x++) {
            ...
            if (x >= 0 && y >= 0) {
                if (zBuffer[x][y] == null || zBuffer[x][y] > Math.abs(z - camera.getPosition().getZ())) {
                    MyColor color = getColor(myColor1, myColor2, myColor3, x, y, x1, x2, x3, y1, y2, y3, image, texturePoint1, texturePoint2, texturePoint3, mesh);
                    ...
                    triangleBuffer[x][y] = triangleIndex;  //new TInfo(mesh, triangleIndex)
                }
            }
        }
    }
